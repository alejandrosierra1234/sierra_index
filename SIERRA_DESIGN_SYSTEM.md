...
Cada etapa tiene figura (rectángulo redondeado, cápsula o rombo redondeado), figura blanca y distintivo opcional con color de la paleta SIERRA. El distintivo es un ícono del catálogo compartido o numeración automática (1, 01, romana, alfabética) o personalizada de hasta cuatro caracteres. Siempre se aloja en un cuadrado redondeado. La paleta y la búsqueda de íconos usan el popover de Index; los demás selectores usan pdSelect.

El dibujo SVG integra figuras, conectores, texto e íconos para compartir exactamente el mismo contenido entre vista previa, impresión y exportación. El texto se ajusta en líneas sin truncarlo; el tamaño se adapta a su contenido. El texto de las figuras es negro SIERRA; solo el número o ícono lleva color y su recuadro es neutro. Los procesos antiguos conservan texto y orden. El color anterior se toma como base del distintivo; los tonos pastel se migran al oscuro de la misma familia para mejorar legibilidad. La numeración automática sigue el orden al mover etapas. Los rombos son figuras visuales de la secuencia, sin bifurcaciones configurables.


### Editor simplificado y firmas reutilizables

Separar Datos, Contenido y Firma en vistas dentro del editor; la vista previa siempre permanece disponible. Ocultar la biblioteca de bloques bajo «Agregar bloque» hasta que se solicite. Mantener el guardado automático y un pie con Volver/Descargar; Deshacer aparece solo cuando hay una operación reversible y Guardar ahora se conserva en el menú de acciones. Evitar instrucciones repetidas en cada párrafo.
...
El encabezado de control, los títulos de sección, el índice, las tablas y las firmas deben sentirse como partes de un mismo documento. Los títulos y sus números usan la misma familia sans, con texto oscuro y fondo transparente para que la marca de agua no quede cortada por sus franjas; el acento teal lateral permanece muy estrecho. El índice es una tabla compacta de número, sección y página: sus celdas alinean los datos sin recurrir a líneas guía decorativas. No aplicar bordes superiores gruesos a tarjetas o cuadros de firma. El espacio horizontal dentro de cada firma se distribuye en dos columnas por fila y la línea para firmar conserva su función, sin convertirse en un acento gráfico. Código, revisión y clasificación comparten una fila propia dentro del encabezado inicial; el número de página se reserva para el pie. El encabezado de continuación dice «Política», presenta el título, identidad y código, y ajusta su altura al contenido. Su casilla de logo es ancha pero no alta para que los logos apaisados se lean sin aumentar la franja; los logos cuadrados conservan su proporción. El pie identifica el departamento como área de implementación y muestra la paginación. La palabra diagonal CONFIDENCIAL usa una escala ligeramente mayor sin aumentar su opacidad.

Los logos de empresa mantienen sus proporciones en vista previa y exportación. La imagen se dimensiona por sus proporciones intrínsecas, limitada por el ancho y alto de su casilla; no se le asignan ancho y alto fijos confiando en `object-fit`, porque el capturador de PDF puede estirar el bitmap dentro de ese rectángulo. Antes de capturar el PDF se fijan en el documento temporal ancho y alto proporcionales al archivo cargado, para las casillas inicial y de continuación. Los SVG se convierten desde la imagen ya cargada en la vista previa a una imagen de alta resolución; si no está disponible, se carga el SVG explícitamente y se avisa si falla, en lugar de entregar un logo incompleto. La clasificación CONFIDENCIAL debe leerse claramente en cada página y no depender solo del color.
