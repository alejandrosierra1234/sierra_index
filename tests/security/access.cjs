...
(async () => {
  ...
  let finishRefresh;
  ctx.sb.rpc = () => new Promise(resolve => { finishRefresh = resolve });
  const pendingRefresh = ctx.loadGrants();
  assert.equal(can('read', 'fabric', 'one'), true, 'refresh keeps the last verified grants until the server answers');
  finishRefresh({data: {version: 2, active: true, grants: response.data.grants}});
  await pendingRefresh;
  ctx.sb.rpc = async () => response;
