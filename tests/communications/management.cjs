...
test('editorial circulars preserve their kind, metadata, layouts and shared blocks',()=>{
  ...
  assert.ok(box.querySelector('.news-masthead'));assert.ok(box.querySelector('.news-section-pleca'));assert.ok(box.querySelector('.news-title h1'));assert.equal(box.querySelector('.news-lead h1'),null);assert.ok(box.querySelector('.news-quote'));assert.ok(!box.querySelector('.sierra-department'));
