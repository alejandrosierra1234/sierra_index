...
test('editorial circulars preserve their kind, metadata, layouts and shared blocks',()=>{
  ...
  d.newsPlant='Planta Norte';d.newsDepartment='Finanzas';d.eyebrow='Tema anterior';
  for(const layout of ['image-top','image-right','text']){
    ...
    if(layout!=='text')assert.ok(lead.innerHTML.indexOf('news-author')<lead.innerHTML.indexOf('<figure'));
  ...
  assert.equal(copy.newsPlant,'Planta Norte');assert.equal(copy.newsDepartment,'Finanzas');
...
test('memo PDF pagination does not leave the last word of a paragraph alone',()=>{
  ...
  try{
    ...
    w.HTMLElement.prototype.getBoundingClientRect=function(){const bottom=this.classList.contains('memo-print-content')?88:this.textContent.length;return{top:0,bottom,left:0,right:100,width:100,height:bottom}};
