...
assert.equal(callout.style.getPropertyValue('--info-plaque-ink'),'#0b0b0b')
...
assert.match(infographicCss,/background:var\(--info-family-base/)
assert.doesNotMatch(infographicCss,/color-mix\(/)
...
w.document.querySelector('.info-inspector').scrollTop=120
w.infoOpenDesignPanel()
assert.equal(w.document.querySelector('.info-inspector').scrollTop,0)
assert.equal(w.document.getElementById('info-design-button').getAttribute('aria-pressed'),'true')
assert.match(w.document.getElementById('info-inspector').textContent,/Medida actual/)
w.infoSetPreset('custom')
const sizeDialog=w.document.querySelector('.info-size-dialog')
assert.ok(sizeDialog?.open)
sizeDialog.querySelector('#info-custom-unit').value='px'
sizeDialog.querySelector('#info-custom-unit').dispatchEvent(new w.Event('change',{bubbles:true}))
...
w.infoSetPreset('custom')
const printDialog=w.document.querySelector('.info-size-dialog')
printDialog.querySelector('#info-custom-unit').value='cm'
printDialog.querySelector('#info-custom-unit').dispatchEvent(new w.Event('change',{bubbles:true}))
printDialog.querySelector('#info-custom-width').value='21'
printDialog.querySelector('#info-custom-height').value='29.7'
printDialog.querySelector('.btn-primary').click()
assert.equal(w.document.querySelector('.info-page').style.width,'794px')
assert.equal(w.document.querySelector('.info-page').style.height,'1123px')
assert.match(w.document.getElementById('info-canvas-size').textContent,/21 × 29.7 cm/)
console.log('PASS: Design opens visibly and custom digital or print dimensions update live')
...
let downloaded='',pdfSaved='',captureOptions=null,pdfOptions=null
w.html2canvas=async(_page,options)=>{captureOptions=options;return{toBlob:callback=>callback(new w.Blob(['png'],{type:'image/png'})),toDataURL:()=> 'data:image/jpeg;base64,AA=='}}
w.URL.createObjectURL=()=> 'blob:test';w.URL.revokeObjectURL=()=>{}
w.HTMLAnchorElement.prototype.click=function(){downloaded=this.download}
w.jspdf={jsPDF:function(options){pdfOptions=options;return{internal:{pageSize:{getWidth:()=>210,getHeight:()=>297}},setProperties(){},addImage(){},save(name){pdfSaved=name}}}}
await w.infoExport('png');await w.infoExport('pdf')
assert.match(downloaded,/\.png$/);assert.match(pdfSaved,/\.pdf$/)
assert.equal(captureOptions.scale,300/96)
assert.equal(Array.from(pdfOptions.format).join(','),'210,297')
console.log('PASS: PNG and PDF exports trigger downloads at print resolution and exact physical size')
w.close()
