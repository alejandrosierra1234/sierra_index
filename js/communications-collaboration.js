(() => {
  ...
  var ValidationError = class {
     ...
     */
    extend(path, expected, has, message2 = null) {
      this._rerrs.push({ path, expected, has, message: message2 });
    }
    toString() {
      const s = [];
...
${err.toString()}`);
  ...
  var keyed = (value) => Array.isArray(value) && value.every((item) => object(item) && (item.id || item.uid));
  var identity = (item) => String(item.id || item.uid);
  var textual = (key) => /^(subject|summary|content|text|title|description|name|author|salutation|closing|intro|caption|heroCaption|contactNote)$/.test(key);
  function replaceText(type, before, after) {
    let start = 0;
  ...
  function encode2(value, key) {
    if (typeof value === "string" && textual(key)) return new YText(value);
    if (keyed(value)) {
      const result = new YMap(), items = new YMap(), positions = new YMap();
      result.set("_kind", "list");
      result.set("_items", items);
      result.set("_positions", positions);
      let position = null;
      value.forEach((item) => {
        const id2 = identity(item);
        items.set(id2, encode2(item));
    ...
    if (object(value)) {
      ...
      Object.entries(value).forEach(([k, v]) => {
        if (!forbidden.has(k)) result.set(k, encode2(v, k));
      });
      return result;
    }
  ...
  function decode(value) {
    ...
    if (value instanceof YMap) {
      if (value.get("_kind") === "list") {
        ...
        }).map((id2) => decode(items.get(id2)));
      }
      const result = {};
      value.forEach((v, k) => {
        if (!forbidden.has(k)) result[k] = decode(v);
      });
      return result;
    }
    return structuredClone(value);
  ...
  function patch(type, before, after) {
    ...
    for (const [key, value] of Object.entries(after)) {
      ...
      if (current instanceof YText && typeof previous === "string" && typeof value === "string") {
        replaceText(current, previous, value);
      } else if (current instanceof YMap && current.get("_kind") === "list" && keyed(previous) && keyed(value)) {
        ...
        for (const id2 of old.keys()) if (!next.has(id2)) {
          items.delete(id2);
          positions.delete(id2);
        }
        value.forEach((item) => {
          const id2 = identity(item);
          if (!old.has(id2)) items.set(id2, encode2(item));
          else if (items.has(id2)) patch(items.get(id2), old.get(id2), item);
        });
        if (!same(previous.map(identity), value.map(identity))) {
          let left = null;
          value.forEach((item, i) => {
            const id2 = identity(item);
            if (!items.has(id2)) return;
            let position = positions.get(id2);
            ...
            if (!position || left && position <= left || right && position >= right) {
              position = generateKeyBetween(left, right || null);
              positions.set(id2, position);
            }
            left = position;
          });
        }
      } else if (current instanceof YMap && object(previous) && object(value)) patch(current, previous, value);
      else type.set(key, encode2(value, key));
    }
  }
  var CommunicationModel = class {
    constructor(doc4 = new Doc()) {
      this.doc = doc4;
      this.root = doc4.getMap("communication");
    }
    seed(value) {
      this.doc.transact(() => patch(this.root, {}, value), "local");
    }
    read() {
      return decode(this.root);
    }
    apply(before, after) {
      this.doc.transact(() => patch(this.root, before, after), "local");
    }
  };
  var toBase642 = (bytes) => {
    let binary = "";
  ...
  var fromBase64 = (value) => Uint8Array.from(atob(value.replace(/\s/g, "")), (c) => c.charCodeAt(0));

  // transport.js
  var CommunicationTransport = class {
    constructor({ doc: doc4, rpc, valid, presence, onChange, onStatus }) {
      Object.assign(this, { doc: doc4, rpc, valid, presence, onChange, onStatus });
      ...
      this.role = "viewer";
      this.failures = 0;
      this.listener = (update, origin) => {
        if (origin !== "remote") this.pending.push(update);
      };
      doc4.on("update", this.listener);
    }
    get dirty() {
    ...
    async exchange() {
      ...
      if (!this.batch && count) this.batch = { count, nonce: crypto.randomUUID(), update: toBase642(yjs_exports.mergeUpdates(this.pending.slice(0, count))) };
      try {
        const data = await this.rpc("communication_sync", {
          p_after: this.revision,
          p_nonce: this.batch?.nonce || null,
          p_update: this.batch?.update || null,
          p_session: this.session,
          p_awareness: JSON.stringify(this.presence())
        });
        if (this.stopped || !this.valid()) return;
        this.role = data.role;
        if (this.batch) {
          this.pending.splice(0, this.batch.count);
          this.batch = null;
        }
        this.doc.transact(() => {
          if (data.snapshot) yjs_exports.applyUpdate(this.doc, fromBase64(data.snapshot), "remote");
          for (const update of data.updates) yjs_exports.applyUpdate(this.doc, fromBase64(update.payload), "remote");
        }, "remote");
        ...
        if (!this.dirty && this.role !== "viewer" && this.revision - data.snapshot_revision >= 50) {
          await this.rpc("communication_checkpoint", {
            ...
            p_snapshot: toBase642(yjs_exports.encodeStateAsUpdate(this.doc)),
        ...
        if (error.code === "42501" || error.status === 401 || error.status === 403) {
          this.stop();
          this.onStatus("Ya no tienes acceso a este comunicado.", true);
    ...
    start() {
      const next = async () => {
        ...
        if (!this.stopped && this.valid()) this.timer = setTimeout(next, Math.min(1e4, 650 * 2 ** this.failures));
      };
      next();
    }
    stop() {
      ...
      this.doc.off("update", this.listener);
    }
  };
  ...
  function findDiffEnd(a, b, posA, posB) {
    for (let iA = a.childCount, iB = b.childCount; ; ) {
      ...
      if (childA.isText && childA.text != childB.text) {
        let same2 = 0, minSize = Math.min(childA.text.length, childB.text.length);
        while (same2 < minSize && childA.text[childA.text.length - same2 - 1] == childB.text[childB.text.length - same2 - 1]) {
          same2++;
          posA--;
          posB--;
        }
  ...
  var Mark = class _Mark {
    ...
    */
    static setFrom(marks) {
      ...
      copy4.sort((a, b) => a.type.rank - b.type.rank);
      return copy4;
    }
  };
  Mark.none = [];
  ...
  function normalize(map2) {
    ...
    for (let prop in map2)
      copy4[normalizeKeyName(prop)] = map2[prop];
    return copy4;
  }
  ...
  var yXmlFragmentToProseMirrorFragment = (yXmlFragment, schema2) => {
  ...
  var yXmlFragmentToProseMirrorRootNode = (yXmlFragment, schema2) => schema2.topNodeType.create(null, yXmlFragmentToProseMirrorFragment(yXmlFragment, schema2));
  ...
  var schema = new Schema2({
    ...
    marks: {
      ...
      color: {
        attrs: { color: {} },
        parseDOM: [{ style: "color", getAttrs: (value) => /^#[0-9a-f]{6}$|^rgb\([\d,\s]+\)$/i.test(value) ? { color: value } : false }],
        toDOM: (mark) => ["span", { style: "color:" + mark.attrs.color }, 0]
      }
    }
  });
  function seedRich(fragment, html) {
    if (fragment.length) return;
    const container = document.createElement("div");
    container.innerHTML = html;
    prosemirrorToYXmlFragment(DOMParser2.fromSchema(schema).parse(container), fragment);
  }
  function readRich(fragment, keepBlocks = false) {
  ...
  function bindRich({ element: element2, fragment, awareness, editable, onChange, field }) {
    for (const attr of [...element2.attributes]) if (attr.name.startsWith("on")) element2.removeAttribute(attr.name);
    element2.removeAttribute("contenteditable");
    element2.innerHTML = "";
    const cursorBuilder = (user) => {
      const cursor = document.createElement("span"), label2 = document.createElement("span");
      ...
      label2.textContent = String(user.name || "Colaborador").slice(0, 80);
      cursor.append(label2);
      return cursor;
    };
    const view = new EditorView(element2, {
      ...
      editable,
      attributes: { "aria-label": element2.getAttribute("aria-label") || "Texto del comunicado", "role": "textbox", "aria-multiline": "true" },
      handleDOMEvents: { focus: () => {
        awareness.setLocalStateField("field", field);
        return false;
      } },
  ...
  function richCommand(view, command, value) {
    const { state, dispatch } = view;
    const mark = { bold: "strong", italic: "em", underline: "underline" }[command];
    if (mark) toggleMark(schema.marks[mark])(state, dispatch);
    else if (command === "formatBlock") setBlockType2(value === "p" ? schema.nodes.paragraph : schema.nodes.heading, { level: 3 })(state, dispatch);
      ...
      const { from: from2, to, empty: empty2 } = state.selection, m = schema.marks.color.create({ color: value });
      dispatch(empty2 ? state.tr.addStoredMark(m) : state.tr.addMark(from2, to, m));
  ...
  var label = (role) => ({ owner: "Propietario", editor: "Puede editar", viewer: "Puede leer" })[role] || "Sin acceso";
  var richBlocks = (d) => (d.blocks || []).flatMap((b) => b.type === "cards" ? b.cards || [] : ["text", "quote", "feature"].includes(b.type) ? [b] : []);
  var stripped = (d) => {
    const value = copy3(d);
    richBlocks(value).forEach((b) => {
      delete b.richHtml;
      delete b.content;
    });
    if (value.kind === "aviso") {
      delete value.noticeRichHtml;
      delete value.noticeMessage;
    }
    return value;
  };
  function dialog(title) {
    ...
    el.id = "comms-sharing";
    el.className = "comms-share-dialog";
    const header = document.createElement("header"), heading = document.createElement("h2"), close2 = document.createElement("button");
    heading.id = "comms-sharing-title";
    heading.textContent = title;
    el.setAttribute("aria-labelledby", heading.id);
    close2.className = "icon-btn";
    close2.textContent = "\xD7";
    close2.title = "Cerrar";
    close2.setAttribute("aria-label", "Cerrar");
    close2.onclick = () => el.close();
    header.append(heading, close2);
    el.append(header);
  ...
  function install(api) {
    ...
    const rpc = async (name, args2 = {}) => {
      ...
      if (!account) throw Error("Inicia sesi\xF3n para continuar.");
      const { data, error } = await api.rpc(name, args2);
      if (getAccount() !== account) throw Object.assign(Error("La sesi\xF3n cambi\xF3."), { code: "42501" });
      if (error) throw error;
      return data;
    };
    const active = () => {
      if (session && session.account !== getAccount()) {
        stop();
        api.current(null);
        api.clearPage("La sesi\xF3n cambi\xF3.");
      }
      return !!session;
    };
    ...
    const destroyViews = () => {
      ...
      views.clear();
      inputCleanups.splice(0).forEach((clean) => clean());
    };
    const read = () => {
      const d = session.model.read();
      richBlocks(d).forEach((b) => Object.assign(b, readRich(session.model.doc.getXmlFragment("rich:" + b.id))));
      if (d.kind === "aviso") {
        const rich = readRich(session.model.doc.getXmlFragment("rich:notice-message"), true);
        d.noticeRichHtml = rich.richHtml;
        d.noticeMessage = rich.content;
      }
      return d;
    };
    ...
    const capture = () => {
      ...
      const d = api.current();
      if (!d || d.id !== session.source) return false;
      prepareRich(session.model, d);
      const next = stripped(d);
      session.model.apply(session.baseline, next);
      session.baseline = copy3(next);
      return true;
    };
    const status = (text2, blocked = false) => {
      if (!session) return;
      session.blocked = blocked;
      document.querySelectorAll("#memo-save-state,[data-collab-status]").forEach((el) => el.textContent = text2);
      ...
      if (editable()) {
        lockedElements.forEach((wasDisabled, el) => {
        ...
        if (!el.closest(".memo-studio-actions")) {
          if (!lockedElements.has(el)) lockedElements.set(el, el.disabled);
          el.disabled = true;
        }
      });
    ...
    function stop(closeDialog = true) {
      ...
      if (session) {
        ...
        session.model.doc.destroy();
      }
      session = null;
      deferred = false;
      lockedElements.clear();
      api.clearSelectors();
      if (closeDialog) document.getElementById("comms-sharing")?.close();
    }
    function applyRemote(data) {
      if (!active()) return;
      const previous = api.current(), next = read();
      api.validate(next);
      session.baseline = stripped(next);
      api.current(next);
      const states = session.awareness.getStates();
      const seen = /* @__PURE__ */ new Set([session.model.doc.clientID]);
      data.presence.forEach((person) => {
        if (person.session_id === session.transport.session) return;
        try {
          const state = JSON.parse(person.awareness), id2 = Number(state.clientID);
          if (!Number.isSafeInteger(id2) || id2 === session.model.doc.clientID) return;
          const name = String(person.name || "Colaborador").slice(0, 80), color = colors[id2 % colors.length];
          states.set(id2, { user: { name, color }, field: state.field, cursor: state.cursor });
          seen.add(id2);
        } catch {
        }
      });
      const removed = [];
      states.forEach((_state, id2) => {
        if (!seen.has(id2)) {
          states.delete(id2);
          removed.push(id2);
        }
    ...
    function paintPresence(people = []) {
      const bar = document.querySelector(".comms-collab-bar");
      if (!bar) return;
      bar.replaceChildren();
      const access = document.createElement("span");
      access.textContent = "Privado \xB7 " + label(session.transport.role);
      bar.append(access);
      ...
      people.forEach((p) => {
        ...
        item.className = "comms-collab-person";
        item.textContent = p.name;
        bar.append(item);
      });
      const state = document.createElement("span");
      state.dataset.collabStatus = "";
      state.setAttribute("role", "status");
      state.textContent = session.transport.dirty ? "Guardando cambios..." : "Guardado en Index";
      bar.append(state);
    }
    async function open(id2) {
      if (active() && session.id === id2) return;
      if (active()) {
        capture();
        await session.transport.sync();
        if (session.transport.dirty) throw Error("Todav\xEDa hay cambios pendientes.");
      }
      if (!active() && api.current() && !api.persistLocal()) throw Error("No se pudo guardar el documento local.");
      stop(false);
      api.current(null);
      api.clearPage("Abriendo comunicado privado...");
      const account = getAccount(), model = new CommunicationModel(), awareness = new Awareness(model.doc);
      const current = { id: id2, account, model, awareness, blocked: true, source: null, baseline: {} };
      const transport = new CommunicationTransport({
        doc: model.doc,
        rpc: (name, args2) => rpc(name, { p_id: id2, ...args2 }),
        valid: () => session === current && getAccount() === account,
        presence: () => ({ clientID: model.doc.clientID, ...awareness.getLocalState() }),
        onChange: (data) => {
          if (current.source) applyRemote(data);
        },
        onStatus: status
      });
    ...
    async function share() {
      ...
      try {
        ...
        privacy.className = "comms-collab-lock";
        privacy.textContent = "Privado \xB7 Solo las personas de esta lista tienen acceso.";
        el.append(privacy);
        const rows = document.createElement("div");
        el.append(rows);
        for (const person of access.members) {
          const row = document.createElement("div");
          row.className = "comms-share-member";
          const name = document.createElement("span"), email = document.createElement("small");
          ...
          if (access.role === "owner" && person.role !== "owner") {
            const select = api.select("Acceso de " + person.name, [{ value: "viewer", label: "Puede leer" }, { value: "editor", label: "Puede editar" }, { value: "", label: "Quitar acceso" }], person.role, async (value) => {
              select.inert = true;
              try {
                await rpc("communication_share", { p_id: id2, p_email: person.email, p_role: value || null });
                el.close();
                await share();
              } catch (error) {
                select.inert = false;
                message(el, "No se pudo cambiar el acceso. El permiso anterior se conserva.", true);
              }
            });
            row.append(select);
          } else {
            const role = document.createElement("span");
            role.textContent = label(person.role);
            row.append(role);
          }
          rows.append(row);
        }
        if (access.role === "owner") {
          ...
          form.innerHTML = '<label>Correo de la persona<input name="email" type="email" class="control-input" autocomplete="off" required></label><div data-role-control></div><button class="btn btn-primary" type="submit">Dar acceso</button>';
          form.querySelector("[data-role-control]").append(api.select("Permiso de la invitaci\xF3n", [{ value: "viewer", label: "Puede leer" }, { value: "editor", label: "Puede editar" }], invitedRole, (value) => {
            invitedRole = value;
          }));
          form.onsubmit = async (event) => {
            ...
            try {
              await rpc("communication_share", { p_id: id2, p_email: form.elements.email.value, p_role: invitedRole });
              el.close();
              await share();
            } catch (error) {
              button.disabled = false;
              message(el, error.code === "42501" ? "No tienes permiso para compartir." : "No se pudo dar acceso. Verifica que el correo tenga una cuenta activa en Index.", true);
            }
          };
          el.append(form);
        }
    ...
    async function library() {
      const el = dialog("Comunicados compartidos");
      message(el, "Cargando...");
      try {
        const documents = await rpc("communication_list");
        if (!el.isConnected) return;
        el.querySelector("[data-message]")?.remove();
        if (!documents.length) message(el, "No tienes comunicados compartidos.");
        for (const d of documents) {
          const row = document.createElement("button");
          row.className = "btn btn-ghost comms-share-member";
          row.style.width = "100%";
          row.textContent = d.title;
          const access = document.createElement("small");
          access.textContent = label(d.role);
          row.append(access);
          row.onclick = async () => {
            row.disabled = true;
            try {
              ...
              row.disabled = false;
              message(el, "No se pudo abrir el comunicado. Comprueba tu acceso y conexi\xF3n.", true);
            }
          };
          el.append(row);
        }
      } catch (error) {
        message(el, "El espacio compartido no est\xE1 disponible. No se modificaron tus documentos locales.", true);
      }
    }
    function rendered() {
      ...
      if (header && !header.querySelector("[data-share-access]")) {
        ...
        button.type = "button";
        button.dataset.shareAccess = "";
        button.innerHTML = api.icon("users", 16) + " Compartir acceso";
        button.onclick = share;
        header.prepend(button);
      }
      if (!active()) return;
      destroyViews();
      const panel = document.querySelector(".memo-customizer");
      if (!panel) return;
      if (!panel.querySelector(".comms-collab-bar")) {
        const bar = document.createElement("div");
        bar.className = "comms-collab-bar";
        panel.querySelector(".memo-panel-head").after(bar);
      }
      paintPresence();
      document.querySelectorAll(".memo-rich-editor[data-block-id]").forEach((element2) => {
        const id2 = element2.dataset.blockId, fragment = session.model.doc.getXmlFragment("rich:" + id2);
        const view = bindRich({ element: element2, fragment, awareness: session.awareness, editable, field: id2, onChange: () => {
          if (!active()) return;
          const block = richBlocks(api.current()).find((b) => b.id === id2);
          if (block) {
            Object.assign(block, readRich(fragment));
            api.preview();
          } else if (id2 === "notice-message" && api.current().kind === "aviso") {
            const rich = readRich(fragment, true);
            api.current().noticeRichHtml = rich.richHtml;
            api.current().noticeMessage = rich.content;
            api.preview();
          }
        } });
        views.set(element2, view);
      });
      document.querySelectorAll(".memo-form input,.memo-form textarea").forEach((element2) => {
        const handler = element2.getAttribute("oninput") || "";
        ...
        if (!(type instanceof yjs_exports.Text)) return;
        let anchor = null, head = null;
        const before = () => {
          if (document.activeElement !== element2 || element2.selectionStart === null) return;
          anchor = yjs_exports.createRelativePositionFromTypeIndex(type, Math.min(element2.selectionStart, type.length));
          head = yjs_exports.createRelativePositionFromTypeIndex(type, Math.min(element2.selectionEnd, type.length));
        };
        const changed = () => {
          if (element2.value === type.toString()) return;
          element2.value = type.toString();
          if (document.activeElement === element2 && anchor && head) {
            const a = yjs_exports.createAbsolutePositionFromRelativePosition(anchor, session.model.doc), b = yjs_exports.createAbsolutePositionFromRelativePosition(head, session.model.doc);
            if (a && b) element2.setSelectionRange(a.index, b.index);
          }
        };
        session.model.doc.on("beforeTransaction", before);
        type.observe(changed);
        inputCleanups.push(() => {
          type.unobserve(changed);
          session?.model.doc.off("beforeTransaction", before);
        });
      });
    ...
    function homeRendered() {
      const header = document.querySelector(".comms-header-actions");
      if (!header || header.querySelector("[data-shared-library]")) return;
      const button = document.createElement("button");
      button.className = "btn btn-secondary";
      button.dataset.sharedLibrary = "";
      button.innerHTML = api.icon("users", 16) + " Compartidos";
      button.onclick = library;
      header.prepend(button);
    }
    document.addEventListener("focusout", () => setTimeout(() => {
      if (active() && deferred && !document.activeElement?.closest(".memo-form")) {
        deferred = false;
        api.render();
      }
    }, 0));
    window.addEventListener("beforeunload", (event) => {
      if (active() && session.transport.dirty) {
        event.preventDefault();
        event.returnValue = "";
      }
    });
    return {
      ...
      beforeRender: () => {
        if (active() && !rendering) capture();
        destroyViews();
      },
      persist: () => {
        if (!active()) return null;
        capture();
        session.transport.sync().catch(() => {
        });
        return !session.blocked;
      },
      command: (control, command, value) => {
        const element2 = control.closest(".memo-rich-toolbar")?.nextElementSibling, view = views.get(element2);
        if (!view) return false;
        if (editable()) richCommand(view, command, value);
        return true;
      },
      leave: async () => {
        if (!active()) return true;
        capture();
        if (!session.transport.dirty) {
          stop();
          return true;
        }
        try {
          await session.transport.sync();
          if (session.transport.dirty) return false;
          stop();
          return true;
        } catch {
          return false;
