...
async function renderSampleWorkflow(col,items,knownWorkflow=undefined) {
 ...
 try {
  ...
  root.innerHTML=`<section class="sf-panel" aria-label="Flujo de la colección">
   <div class="sf-heading"><div><h2>${sampleFlowIcon('box')} Flujo de la colección</h2>${!w?'<p>Define las condiciones antes de preparar la colección.</p>':''}</div><button class="btn btn-secondary btn-sm" onclick="showCollectionRecord('${col.id}')" title="Actualizar colección">${sampleFlowIcon('refresh')} Actualizar</button></div>
   ...
   ${stage==='selection'?sampleWorkflowSettings(col,w,sales):`<p class="sf-hint">${destination==='internal'?'Entrega interna · Packing list visible para Ventas':'Cliente o marca · Aprobación de Ventas obligatoria'}${w.costing_required?' · Costing requerido':''}</p>${w.brief?`<details><summary>${sampleFlowIcon('list')} Instrucciones</summary><p class="sf-brief">${esc(w.brief)}</p></details>`:''}`}
...
function sampleWorkflowSettings(col,w,sales){
 const destination=w?.destination_kind||'client';
 return `<details class="sf-settings"><summary>${sampleFlowIcon('people')} Destino y condiciones <small>${w?.configured?esc(w.destination_country||'Configuradas'):'Pendiente de configurar'}</small></summary>
 <fieldset class="sf-settings-grid" ${sales?'':'disabled'}><legend class="sr-only">Condiciones de la colección</legend>
 <div><fieldset><legend>¿Quién recibe?</legend><div class="sf-options">${[['client','Cliente'],['brand','Marca'],['internal','Equipo interno']].map(([key,label])=>`<label><input type="radio" name="sf-destination" value="${key}" ${destination===key?'checked':''}>${label}</label>`).join('')}</div></fieldset><p class="sf-hint">Clientes y marcas requieren aprobación del packing list. Las entregas internas no requieren aprobación de Ventas.</p></div>
 <div><label class="sf-label" for="sf-country">País de destino</label><input id="sf-country" maxlength="100" value="${esc(w?.destination_country||'')}" placeholder="Ej. Guatemala"><label class="sf-check"><input id="sf-costing" type="checkbox" ${w?.costing_required?'checked':''}> Completar costing antes de preparar y etiquetar</label></div>
 <div class="sf-span-all"><label class="sf-label" for="sf-brief">Instrucciones para PD</label><textarea id="sf-brief" rows="3" placeholder="Aplicación, referencias, colores y fecha de presentación…">${esc(w?.brief||'')}</textarea></div>
 <details class="sf-span-all"><summary>${sampleFlowIcon('list')} Opciones de etiqueta <small>Opcional</small></summary><label class="sf-label" for="sf-label-title">Encabezado de etiqueta</label><input id="sf-label-title" maxlength="150" value="${esc(w?.label_title||'')}" placeholder="Nombre de la colección"><label class="sf-check"><input id="sf-label-price" type="checkbox" ${w?.label_show_price?'checked':''}> Incluir precio y condiciones en la etiqueta</label></details>
 ${sales?`<div class="sf-form-actions sf-span-all"><button class="btn btn-secondary" onclick="saveSampleWorkflow('${col.id}',this)">${sampleFlowIcon('save')} Guardar condiciones</button></div>`:''}
 </fieldset></details>`;
}
