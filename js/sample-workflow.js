...
async function renderSampleWorkflow(col,items,knownWorkflow=undefined) {
 ...
 try {
  ...
  root.innerHTML=`<section class="sf-panel" aria-label="Flujo de la colección">
   ...
   ${stage==='selection'?`<details ${!w?.configured?'open':''}><summary>${sampleFlowIcon('people')} Destino y condiciones</summary><fieldset ${sales?'':'disabled'}><legend>¿Quién recibe la colección?</legend><div class="sf-options">${[['client','Cliente'],['brand','Marca'],['internal','Equipo interno']].map(([key,label])=>`<label><input type="radio" name="sf-destination" value="${key}" ${destination===key?'checked':''}>${label}</label>`).join('')}</div><p class="sf-hint">Clientes y marcas requieren aprobación de Ventas antes de la salida. Las entregas internas siempre llevan packing list.</p><label class="sf-label" for="sf-country">País de destino</label><input id="sf-country" maxlength="100" value="${esc(w?.destination_country||'')}" placeholder="Ej. Guatemala"><label class="sf-check"><input id="sf-costing" type="checkbox" ${w?.costing_required?'checked':''}> Completar costing antes de preparar y etiquetar</label><label class="sf-label" for="sf-brief">Necesidad del cliente o instrucciones para PD</label><textarea id="sf-brief" rows="3" placeholder="Aplicación, referencias, colores y fecha de presentación…">${esc(w?.brief||'')}</textarea><label class="sf-label" for="sf-label-title">Encabezado de etiqueta · opcional</label><input id="sf-label-title" maxlength="150" value="${esc(w?.label_title||'')}" placeholder="Nombre de la colección"><label class="sf-check"><input id="sf-label-price" type="checkbox" ${w?.label_show_price?'checked':''}> Incluir precio y condiciones en la etiqueta</label>${sales?`<button class="btn btn-secondary btn-sm" onclick="saveSampleWorkflow('${col.id}',this)">${sampleFlowIcon('save')} Guardar condiciones</button>`:''}</fieldset></details>`:`<p class="sf-hint">${destination==='internal'?'Entrega interna · Packing list visible para Ventas':'Cliente o marca · Aprobación de Ventas obligatoria'}${w.costing_required?' · Costing requerido':''}</p>${w.brief?`<details><summary>${sampleFlowIcon('list')} Instrucciones</summary><p class="sf-brief">${esc(w.brief)}</p></details>`:''}`}
   ${w?`<div id="sf-packing"></div><div class="sf-next">${stage==='selection'?`<p>${col.status==='draft'?'Completa y envía la selección usando el formulario de abajo.':'Confirma que la selección está completa.'} ${w.costing_required?'Revisa los precios antes de liberar.':''}</p>${sales&&col.status!=='draft'?`<button class="btn btn-primary" onclick="runSampleWorkflow('${col.id}','release',this)">${sampleFlowIcon('check')} Liberar para PD</button>`:''}`:''}
