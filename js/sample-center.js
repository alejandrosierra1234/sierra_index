...
function returnToSampleCenter(){return showSampleCenter(sampleCenterState.view,true)}
...
async function showSampleCenter(view='work',preserve=false){
 ...
 const root=showSampleTool('Centro de muestras','Colecciones, pendientes y entregas de Ventas y PD',view==='collections'?'samples|_|collections':'samples|_|requests');
...
async function loadSampleCenter(){
 ...
 try{
  const {data,error}=await sb.rpc('sample_center',{p_view:sampleCenterState.view,p_search:sampleCenterState.search,p_offset:sampleCenterState.offset,p_limit:sampleCenterState.limit});
...
function openSampleCenterCollection(id){sampleCenterState.request++;showCollectionRecord(id,'returnToSampleCenter')}
