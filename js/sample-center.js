...
const sampleCenterTabs=[['work','list','Mi trabajo'],['collections','package','Mis colecciones'],['inventory','box','Inventario'],['tracking','truck','Seguimiento']];
...
function sampleCenterRow(c){
 ...
 return `<tr><td><button class="sc-record" onclick="openSampleCenterCollection('${c.id}')">${esc(name)}</button><small>${c.name?esc(c.collection_id)+' · ':''}${!sampleActiveSpace?esc(sampleSpaces[c.sample_space]?.label||(c.sample_space==='mixed'?'Histórica · varios equipos':'Por clasificar'))+' · ':''}${c.item_count} referencia${c.item_count===1?'':'s'}${c.fabric_count?' · Tela':''}${c.garment_count?' · Prendas':''}</small><small>${esc(c.customer||c.recipient||'Destinatario pendiente')} · ${esc(dest)}${c.destination_country?' · '+esc(c.destination_country):''}</small></td><td><span class="sc-stage ${task.tone}">${centerIcon(({selection:'list',preparing:'box',packing_review:'file',packed:'package',shipped:'truck',delivered:'check',unfulfilled:'alert-triangle'})[c.stage]||'clock')} ${esc(task.stage)}</span><small class="sc-next-owner">${esc(task.team)} · ${esc(task.detail)}</small>${c.missing_count?`<small>${c.missing_count} ${c.missing_count===1?'renglón':'renglones'} con faltantes</small>`:''}</td><td>${esc(c.owner_name)}${sampleCenterDate(c)}</td><td><button class="btn btn-secondary" onclick="openSampleCenterCollection('${c.id}')" aria-label="${esc(task.active?task.action:'Ver colección')}: ${esc(name)}">${esc(task.active?task.action:'Ver colección')} ${centerIcon('arrow-right')}</button></td></tr>`;
}
function collectionDestinationLabel(c){
  const dest=({client:'Cliente',brand:'Marca',internal:'Equipo interno'})[c.destination_kind]||'Destino por definir';
  return `${dest}${c.destination_country ? ` · ${c.destination_country}` : ''}`
}
function sampleCenterCardHtml(c){
  const task=sampleCenterTask(c)
  const name=c.name||c.collection_id||'Colección sin nombre'
  const created=new Date(c.created_at)
  const createdLabel=created.toLocaleDateString('es-GT',{day:'2-digit',month:'short'})
  const deadline=sampleCenterDate(c)
  const toneCls=`is-${task.tone || 'neutral'}`
  return `<article class="sc-board-card ${toneCls}">
    <button class="sc-board-title" onclick="openSampleCenterCollection('${c.id}')" title="Abrir ${escAttr(name)}">
      <span class="sc-board-title-main">${esc(name)}</span>
      <span class="sc-board-title-sub">${esc(c.collection_id)} · ${createdLabel}</span>
    </button>
    <div class="sc-board-chip ${toneCls}">${centerIcon(({selection:'list',preparing:'box',packing_review:'file',packed:'package',shipped:'truck',delivered:'check',unfulfilled:'alert-triangle'})[c.stage]||'clock')} ${esc(task.stage)}</div>
    <div class="sc-board-meta">
      <div>${esc(collectionDestinationLabel(c))}</div>
      <div>${esc(c.owner_name)} · ${esc(c.recipient || 'Destinatario pendiente')}</div>
      <div>${task.team}</div>
    </div>
    <div class="sc-board-meta" style="margin-top:.45rem">${esc(task.detail)}</div>
    <div class="sc-board-row">${esc(c.item_count)} referencia${c.item_count===1?'':'s'}${c.missing_count?` · ${c.missing_count} renglón${c.missing_count===1?'':'es'} con faltante${c.missing_count===1?'':'s'}`:''}</div>
    <div class="sc-board-date">${deadline}</div>
    <div class="sc-board-footer">
      <button class="btn btn-secondary btn-sm" onclick="openSampleCenterCollection('${c.id}')" aria-label="${esc(task.active?task.action:'Ver colección')}: ${esc(name)}">${esc(task.active?task.action:'Ver colección')} ${centerIcon('arrow-right')}</button>
      <span class="sc-board-status">${esc(task.stage)}</span>
    </div>
  </article>`
}
async function showSampleCenter(view='work',preserve=false){
 ...
 const root=showSampleTool('Muestras · '+sampleSpaceLabel(),view==='collections'?'Mis colecciones: gestión de solicitudes, envíos y seguimiento':'Colecciones, pendientes y entregas del equipo',view==='collections'?'samples|_|collections':'samples|_|requests');
 ...
 if(view==='inventory'){
  ...
  const intro={work:['Qué necesita tu atención','Pendientes que puedes atender con tus permisos. Las fechas vencidas aparecen primero.'],collections:['Tus colecciones','Organiza solicitudes por cliente y estado visual como en un board.'],tracking:['Salidas y cierres','Consulta la guía, el contenido y los faltantes de cada colección.']}[view];
...
async function loadSampleCenter(){
 ...
 const view = sampleCenterState.view
 ...
 try{
  ...
  if (view === 'collections') {
    root.innerHTML=data.rows.length
      ? `<div class="sc-collection-board" aria-label="Listado de colecciones">${data.rows.map(sampleCenterCardHtml).join('')}</div><div class="sc-pagination"><span>${start}–${end} de ${data.total} ${data.total===1?'colección':'colecciones'}</span><div><button class="btn btn-secondary" ${sampleCenterState.offset?'':'disabled'} onclick="pageSampleCenter(-1)">${centerIcon('chevron-left')} Anterior</button><button class="btn btn-secondary" ${end<data.total?'':'disabled'} onclick="pageSampleCenter(1)">Siguiente ${centerIcon('chevron-right')}</button></div></div>`
      : `<div class="sc-empty">${centerIcon('check')}<h3>${sampleCenterState.search?'No encontramos coincidencias':'Todavía no hay colecciones'}</h3><p>${sampleCenterState.search?'Prueba con otro cliente, colección o destino.':'Crea o continúa tus borradores para empezar a construir colecciones.'}</p>${sampleCenterState.view==='work'?'<button class="btn btn-secondary" onclick="showSampleCenter(\'collections\')">Ver mis colecciones</button>':''}</div>`;
  } else {
    root.innerHTML=data.rows.length?`<div class="sf-table-wrap"><table class="sf-table sc-table"><caption class="sr-only">Colecciones y siguiente acción</caption><thead><tr><th>Colección / destino</th><th>Etapa y siguiente paso</th><th>Responsable / fecha requerida</th><th><span class="sr-only">Acción</span></th></tr></thead><tbody>${data.rows.map(sampleCenterRow).join('')}</tbody></table></div><div class="sc-pagination"><span>${start}–${end} de ${data.total} ${data.total===1?'colección':'colecciones'}</span><div><button class="btn btn-secondary" ${sampleCenterState.offset?'':'disabled'} onclick="pageSampleCenter(-1)">${centerIcon('chevron-left')} Anterior</button><button class="btn btn-secondary" ${end<data.total?'':'disabled'} onclick="pageSampleCenter(1)">Siguiente ${centerIcon('chevron-right')}</button></div></div>`:`<div class="sc-empty">${centerIcon('check')}<h3>${sampleCenterState.search?'No encontramos coincidencias':sampleCenterState.view==='work'?'No tienes acciones pendientes':sampleCenterState.view==='tracking'?'Todavía no hay salidas o cierres':'Todavía no hay colecciones'}</h3><p>${sampleCenterState.search?'Prueba con otro cliente, responsable o referencia.':sampleCenterState.view==='work'?'Puedes consultar el avance del equipo en Mis colecciones. Los borradores sin referencias están allí.':'Los registros aparecerán aquí conforme avance el trabajo.'}</p>${sampleCenterState.view==='work'?'<button class="btn btn-secondary" onclick="showSampleCenter(\'collections\')">Ver mis colecciones</button>':''}</div>`;
  }
