...
function infoBlockHtml(b,interactive){
  ...
  const common=`class="info-block-shell info-clickable ${infoSelected===b.id&&interactive?'info-selected':''}" data-info-block="${infoAttr(b.id)}" style="--info-span:${Math.min(12,Math.max(4,Number(b.span)||12))};--info-min-height:${Math.max(0,Number(b.minHeight)||0)}px"${drag}`
  const style=`--info-card-bg:${infoAttr(b.background)};--info-card-ink:${infoAttr(b.color)}`
  let body=''
...
function infoContentInspector(b){
  ...
  if(b.type==='features')return`<section class="info-inspector-section"><span class="info-section-title">Contenido</span>${infoInput('Título','heading',b.heading)}<div class="info-feature-editors">${(b.items||[]).map((item,i)=>{const palette=INFO_ICON_COLORS[i%INFO_ICON_COLORS.length];return`<div class="info-item-editor"><div class="info-item-head"><b>Tarjeta ${i+1}</b><button class="icon-btn" aria-label="Eliminar tarjeta ${i+1}" ${b.items.length<=2?'disabled':''} onclick="infoFeatureRemove('${b.id}','${item.id}')">${infoIcon('trash',14)}</button></div>${infoIconTrigger(item.icon,b.id,item.id)}<div class="info-color-pair">${infoColorControl('Color del ícono',item.iconColor||palette.color,'item',b.id,'iconColor',item.id)}${infoColorControl('Fondo del ícono',item.iconBackground||palette.background,'item',b.id,'iconBackground',item.id,INFO_COLORS)}</div><input class="control-input" aria-label="Título" value="${infoAttr(item.title)}" oninput="infoFeatureSet('${b.id}','${item.id}','title',this.value)"><textarea class="control-input" rows="2" aria-label="Descripción" oninput="infoFeatureSet('${b.id}','${item.id}','text',this.value)">${infoEscape(item.text)}</textarea></div>`}).join('')}</div>${b.items.length<4?`<button class="btn btn-secondary" onclick="infoFeatureAdd('${b.id}')">${infoIcon('plus',15)} Agregar tarjeta</button>`:''}</section>`
...
window.infoFeatureAdd=function(id){const b=infoDraft.blocks.find(x=>x.id===id);if(!b||b.items.length>=4)return;const palette=INFO_ICON_COLORS[b.items.length%INFO_ICON_COLORS.length];b.items.push({id:infoId('item'),icon:'check',title:'Nuevo punto',text:'Agrega una explicación breve.',...palette});infoRenderAll();infoScheduleSave()}
