...
function infoPageHtml(interactive=true){
  const clickHeader=interactive?' onclick="infoSelect(\'header\')"':'' ,clickFooter=interactive?' onclick="infoSelect(\'footer\')"':'',pageFamily=infoFamily(infoDraft.colorFamily),background=infoDraft.canvasTone==='smoke'?'#f5f5f5':'#ffffff',hasFill=infoDraft.blocks.some(block=>!block.hidden&&block.heightMode==='fill')
  return`<article class="info-page" id="info-page" style="width:${infoDraft.width}px;height:${infoDraft.height}px;background:${background};--info-title:#0b0b0b;--info-muted:#6b6b73;--info-accent:${pageFamily.base};--info-accent-dark:${pageFamily.dark};--info-gap:${infoDraft.gap}px">
    <div class="info-page-inner">
      ...
      <main class="info-grid ${hasFill?'has-fill':''}">${infoDraft.blocks.filter(b=>!b.hidden).map(b=>infoBlockHtml(b,interactive)).join('')}</main>
