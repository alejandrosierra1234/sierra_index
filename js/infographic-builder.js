...
function infoIconPicker(value,handler){return`<div class="info-icon-grid" role="group" aria-label="Elegir ícono">${INFO_ICONS.map(([key,label])=>`<button type="button" class="info-icon-choice" title="${infoAttr(label)}" aria-label="${infoAttr(label)}" aria-pressed="${value===key}" onclick="${handler(key)}">${infoIcon(key,19)}</button>`).join('')}</div>`}
...
function infoContentInspector(b){
  ...
  if(['callout','stat'].includes(b.type))fields+=infoField('Ícono',infoIconPicker(b.icon,key=>`infoSetBlock('${b.id}','icon','${key}')`))+`<div class="info-field-row">${infoField('Color del ícono',`<input type="color" value="${infoAttr(b.iconColor||'#007d73')}" oninput="infoSetBlock('${b.id}','iconColor',this.value)">`)}${infoField('Fondo del ícono',`<input type="color" value="${infoAttr(b.iconBackground||'#d9f7f3')}" oninput="infoSetBlock('${b.id}','iconBackground',this.value)">`)}</div>`
